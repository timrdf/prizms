...
if [[ ${0%install.sh} == $0 ]]; then # $0 is 'bash' etc when bootstrapping, it is the path of the script otherwise.
   ...
   if [[ "$1" == "--proj-home" ]]; then
      if [[ ${#2} -gt 0 && "$2" != --* ]]; then
         echo "accepting install.sh --proj-home with $1 and $2" >&2
   ...
   if [[ -z "$i_am_project_user" && \
      ...
      if [[ "$install_project_user" == [yY] ]]; then
         echo $PRIZMS_HOME/bin/install/project-user.sh $home_flag $project_user_name >&2
         ...
         pushd $repodir &> /dev/null
            ...
            if [[ -z "$i_am_project_user" ]]; then # Running as developer e.g. jsmith not loxd
               ...
               if [[ ! -e $www/lodspeakr ]]; then
                  lodspeakr_install='http://lodspeakr.org/install-http' # TODO: appending '-http' used to work.
            ...
            #
            # Note that this depends on the production user to be set up already.
            # Although it could be done without it, it's a bit easier by avoiding some path-hacking.
            if [[ -z "$i_am_project_user" && -h $www/lodspeakr/settings.inc.php && -e $PROJECT_PRIZMS_HOME/lodspeakrs ]]; then  # Running as production user e.g. loxd not jsmith 
