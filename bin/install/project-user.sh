...
if [[ "$1" == '--home-base' ]]; then
   if [[ ${#2} -gt 0 ]]; then
      ...
      echo "`basename $0` accepting adjusted user home via argument: $project_user_home_base" >&2
      shift
   else
      echo "WARNING `basename $0` '--home-base' did not have value; ignoring and using default home: \"$project_user_home_base\"" >&2
   fi
   shift
fi
